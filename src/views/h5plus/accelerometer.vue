<template>
  ...
  >
    ...
    <f7-block>
      ...
      <p>
        x轴方向的加速度：
        <span v-text="xAxis"></span>
      </p>
      <p>
        y轴方向的加速度：
        <span v-text="yAxis"></span>
      </p>
      <p>
        z轴方向的加速度：
        <span v-text="zAxis"></span>
      </p>
    </f7-block>
  </f7-page>
</template>
...
export default {
  ...
  methods: {
    ...
    getCurrentAcceleration () {
      var that = this
      if (that.plusReady) {
        window.plus.accelerometer.getCurrentAcceleration(function (a) {
          that.xAxis = a.xAxis
          that.yAxis = a.yAxis
          that.zAxis = a.zAxis
        })
    ...
    watchAcceleration () {
      ...
      if (that.watchId == null) {
        that.watchId = window.plus.accelerometer.watchAcceleration(
          function (a) {
            that.xAxis = a.xAxis
            that.yAxis = a.yAxis
            that.zAxis = a.zAxis
    ...
    clearWatch () {
      ...
      that.watching = ''
      if (that.watchId) {
        window.plus.accelerometer.clearWatch(that.watchId)
      }
      that.watchId = null
    }
  }
